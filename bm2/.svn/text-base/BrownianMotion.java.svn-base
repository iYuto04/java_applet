...
/* modified version 0.1  2009.07.31*/ 
/* modified version 0.2  2009.08.06*/ 
...
/*-------------------------------------- define gloval -----*/
  ...
  Dimension dim;
  Image imgOff,imgTt,imgGrp;
  Graphics gOff,gTt,gGrp;
  ...
  int Nmtmax=500+1;              /* number of molecules    */
  int NN = Nmtmax + 1;
  double t = 0.0;              /* time (s)               */
  ...
  double pret =0.0; /*previous time for making ensemble */
  double edt =100.0*1.0e-12; /*time interval for making ensemble */
  ...
  double zMax = 0.0;           /* z-Box aize in (m)      */

  double pxx =xMax/2; /*reference x position for ms*/
  double pyy =yMax/2; /*reference y position for ms*/
  int ipxx=wakuX/2+30;
  int ipyy=wakuY/2+70;
  double AU = 1.661e-27;
  double EE = 1.602e-19;
...
/*---------GUI for changing radius--by Ishii--*/
     rChoice = new Choice();
     rChoice.addItem("50.0");
     rChoice.addItem("30.0");
     rChoice.addItem("10.0");
    add(rChoice);
...
/*---------GUI for changing time step --by Ishii--*/
    ...
    imgTt =  createImage(200,200);
    imgGrp =  createImage(450,450);
    gOff = imgOff.getGraphics();
    gTt = imgTt.getGraphics();
    gGrp = imgGrp.getGraphics();
    ...
    pnl.add(new Label(" Radius[10^(-10)m]")); 
    /* pnl.add(new Label("  "));
    pnl.add(new Label("  "));*/
    add(pnl,"North");
  ...
  setInitialPosition();
    ...
    gTt.setColor(Color.white);
    gTt.fillRect(0,0,200,200); 
    gGrp.setColor(Color.white);
    gGrp.fillRect(0,0,450,450); 
         t=0;
         pret=0; 
         times=1;
      for (i=0; i<101; i++) {
         ...
         pxx =xMax/2; /*reference x position for ms*/
         pyy =yMax/2; /*reference y position for ms*/
  }
  ...
  if(e.getSource() == rChoice) {
  String st=rChoice.getSelectedItem();
  Molec[9][3]=Double.parseDouble(st)*AA;
  sigm[9]=Double.parseDouble(st)*AA;
  }
  }
  ...
  public void paint(Graphics g) {
    ...
    g.drawImage(imgTt,650,100,this);
    g.drawImage(imgGrp,650,350,this);
  }

/* ----------------------------- offPaint -------------------- */

  void offPaint() {
    int i,ix,iy,ir,r2,tte,irx,iry,iiX,iiY,ipxx2,ipyy2,irr;
    double tmp,ir2,rotr;
    ...
    gGrp.setColor(Color.black);
    gGrp.drawRect(0,0,300,300);
     gGrp.drawString("0",0,320);
     gGrp.drawString("50",140,320);
     gGrp.drawString("time[ps]",130,340);
     gGrp.drawString("100",280,320);
     gGrp.drawString("120*120",305,10);
      gGrp.drawString("MSD[ang*ang] ",305,150); 
    ...
    ipxx = (int)(40*pxx/xMax);
    ipyy = (int)(40*pyy/yMax);
    iiX = (int)(paintSizeX*pxx/xMax)+30;
    iiY = (int)(paintSizeY*pyy/yMax)+70;
    ipxx2 = (int)(40*xx[i]/xMax);
    ipyy2 = (int)(40*yy[i]/yMax);
    r2=(ipxx-ipxx2)*(ipxx-ipxx2)+(ipyy-ipyy2)*(ipyy-ipyy2);
    gTt.setColor(Color.black);
    gTt.drawString("cont T="+contTemp+"K",0,20);
    gTt.setColor(Color.red);
    gTt.drawString("t="+Math.floor(t*1.0e12)+" ps",0,40);
    gTt.drawString("T="+temperature()+" K",0,60);
    gTt.setColor(Color.black);
    gTt.drawString("box A= 600 ang.",0,80);
    gTt.drawString("1 ang.=1.0x10^(-10) m",0,100);
    gTt.drawString("1 ps=1.0x10^(-12) s",0,120);
    gTt.drawString("1 fs=1.0x10^(-15) s",0,140);
    gOff.setColor(Color.red);
    gOff.fillOval(iiX-ir/20,iiY-ir/20, ir/10, ir/10);
    tte=(int)(100*((t-pret-dt)/edt));
    ir2=(xx[i]-pxx)*(xx[i]-pxx)+(yy[i]-pyy)*(yy[i]-pyy);
    if(times==1){mms[tte]=ir2;}
    if(times>1) {mms[tte]=((times-1)*mms[tte]+ir2)/times;}
    mms[100]=mms[99]; 
    irr=(int)(1600*mms[tte]/(xMax*yMax));
    gTt.setColor(Color.red);
    gTt.drawString("Samples="+times,0,160);
    gTt.drawString("Sampling time="+tte+" ps",0,180);
    gGrp.setColor(Color.red);
     gGrp.fillOval(3*tte,295-irr,5,5); 
    }
...
/*-------------------------------  molecules motion   ---------*/
  void setInitialPosition(){
    ...
    for (i=0; i<Nmt-1; i++) {
      kind[i] = GAS1;
      xx[i] = s*1.0*(double)(i%16)+2.5*s;
      yy[i] = s*1.0*(double)(i/16)+2.5*s;
      if(i>Nmt/4) {
      xx[i] = xMax-s*1.0*(double)(i%16)-2.5*s;
      yy[i] = s*1.0*(double)(i/16)+2.5*s;
                  }
      if(i>Nmt/2) {
      xx[i] = s*1.0*(double)(i%16)+2.5*s;
      yy[i] = yMax-s*1.0*(double)(i/16)-2.5*s;
                  }
      if(i>3*Nmt/4) {
      xx[i] = xMax-s*1.0*(double)(i%16)-2.5*s;
      yy[i] = yMax-s*1.0*(double)(i/16)-2.5*s;
                  }
      vx[i] = 8.0*Math.random()-4.0;
      vy[i] = 8.0*Math.random()-4.0;
      ffx[i] = 0.0;
      ffy[i] = 0.0;
        }
    i = Nmt-1;
